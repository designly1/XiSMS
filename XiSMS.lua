...
-- Flag to control whether SMS sending is enabled
local send_sms_enabled = true

local send_sms = function(message)
    if not send_sms_enabled then
...
windower.register_event('addon command', function(command)
    ...
    elseif command == 'smson' then
        send_sms_enabled = true
        notice('SMS sending enabled')
    elseif command == 'smsoff' then
        send_sms_enabled = false
        notice('SMS sending disabled')
    elseif command == 'help' then
        ...
        windower.add_to_chat(17, '    smson      - Enables SMS sending')
        windower.add_to_chat(17, '    smsoff     - Disables SMS sending')
